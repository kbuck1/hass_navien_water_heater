...
class NavienWaterHeaterMgppEntity(NavienBaseEntity, WaterHeaterEntity):
    ...
    def _get_legacy_unique_id(self) -> str:
        ...
        return f"{self.device.mac_address}_wh"
    ...
    def is_away_mode_on(self):
        # MGPP vacation = 5
        return self.device.channel_status.get("dhwOperationSetting", 0) == 5

    @property
    def current_operation(self):
        mode = self.device.channel_status.get("dhwOperationSetting", 6)
        mode_map = {
            0: STATE_OFF,  # standby
            1: STATE_HEAT_PUMP,
            2: STATE_ELECTRIC,
    ...
    def current_temperature(self):
        ...
        return self.device.dhw_temperature

    @property
    def target_temperature(self):
        """Target DHW temperature in Celsius."""
        return self.device.dhw_temperature_setting
    ...
    def min_temp(self):
        ...
        return self.device.dhw_temperature_min

    @property
    def max_temp(self):
        """Maximum temperature in Celsius."""
        return self.device.dhw_temperature_max

    async def async_set_temperature(self, **kwargs):
        target_c = kwargs.get(ATTR_TEMPERATURE)
        await self.device.set_temperature(target_c)

    async def async_turn_away_mode_on(self):
        # Vacation - use the vacation_days value from the device
        await self.device.set_operation_mode(5)

    async def async_turn_away_mode_off(self):
        # Default back to heat pump
        await self.device.set_operation_mode(1)
    ...
    async def async_set_operation_mode(self, operation_mode):
        ...
        await self.device.set_operation_mode(mode_map.get(operation_mode, 1))
